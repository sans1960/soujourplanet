...
<div class="container mx-auto">
    <div class="w-1/2 p-4 mx-auto mt-5 ">
        <h1 class="mt-5 text-xl text-center font-patua-one"> Edit Article</h1>
        <form action="{{ route('admin.articles.update',$article) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input id="" type="text" class="w-full p-2 mt-5 border-2 border-blue-500 rounded-lg " name="title" value="{{ $article->title }}" placeholder=" " required  autofocus>
            ...
            <input type="text" name="caption" id="" value="{{ $article->caption }}" placeholder="" class="w-full p-2 mt-5 border-blue-500 rounded-lg ">
            <div class="w-full h-auto mt-5 p-2 bg-cover">
                <img id="preview-image-before-upload" src="{{ asset('storage/articles/'.$article->image) }}"
                alt="preview image" >
            </div>
